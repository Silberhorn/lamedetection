...
lanedetection::lanedetection(Mat frame)
{
	...
	k = getStructuringElement(MORPH_RECT, Size(5, 5)); // morph_close (9, 9) bei 640x360 9,9!
	...
	theta			= CV_PI / 180;
	threshold_hough	= 30;
	minLinLength	= 55; // 640x360 110
	maxLineGap		= 30;  //  640x360 60
...
int lanedetection::getSteeringAngle()
{
	...
	middle_point_offset = ((middle[0] + middle[2]) / 2) - (IMAGEWIDTH / 2);
	cout << "Middle_point_offset vorher: " << middle_point_offset << endl;
	// Faktor und Offset
	middle_point_offset = middle_point_offset * 0.80 + 90;	// 0.28 bei aufloesung 640x480 0.35  bei aufloesung  320x180  0.80!

	//cout << "Steering: " << middle_point_offset << endl;

	if(middle_point_offset < 30) {
		middle_point_offset = 30;
	}
	if(middle_point_offset > 150) {
		middle_point_offset = 150;
	}
